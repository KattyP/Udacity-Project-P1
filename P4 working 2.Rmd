...
```{r message=FALSE, warning=FALSE, StatedMonthlyIncome}
...
Albiet a low number of loans, it's interesting that some people who got a loan have zero income or are not employed.  I will look into what people who have $0 income got their loan for and what is the risk rating of their loan. 
...
```{r message=FALSE, warning=FALSE, IncomeRange}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, debt_to_income_ratio}
...
```{r message=FALSE, warning=FALSE, loan_status}
...
```{r message=FALSE, warning=FALSE, LoanOriginalAmount}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, ProsperRationg}
...
#Most loans have zero recommendations. Does having recommendations have an impact on the risk rating?
...
ggplot(aes(x = LenderYield), data = loanData) + 
  geom_histogram(binwidth = 0.005, fill = "blue") + xlim(-0.01,0.4)

summary(loanData$LenderYield )

with(subset(loanData, LenderYield <= 0), table(LenderYield))
#look into the number of people who lost money
```
...
The average lender yield peaked in 2011 before falling to 2014. 

Interestingly the debt to income ratio was highest in 2007, right before the Global Financial Crisis of 2008.
...
Borrowers who are not employed and to a lesser extent retired borrowers carry a higher risk rating. The rating distribution of employed borrowers (including part-time and full-time) have a higher rating (on average) indicating a lower risk loan.
...
Loans with a lower debt to income ratio have a higher prosper rating on average, representing a lower risk loan. 
...
For the purpose of this analysis I've taken out loans with a status as 'Current' this is because I would like to better understand loans what are the factors that effect the outcome of the loan.
...
The line shows where estimated effective yield is equal to lender yield.  It appears that for the large majority of the loans lender yield outperformed the estimated effectice yield.
