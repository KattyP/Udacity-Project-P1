...
```{r Stated Montly Income 2, message=FALSE, warning=FALSE}
...
```{r Loan Status, message=FALSE, warning=FALSE}
...
```{r Loan Original Amount, message=FALSE, warning=FALSE}
...
ggplot(aes(x = LenderYield), data = loanData) + 
  geom_histogram(binwidth = 0.01, fill = "blue") + xlim(-0.01,0.4)
...
Borrowers who are not employed and to a lesser extent retired borrowers carry a higher risk rating. The rating distribution of employed borrowers (including part-time and full-time) on average have a higher rating, indicating a lower risk loan.


```{r Employment by Risk Rating, message=FALSE, echo=FALSE, warning=FALSE}
...
```{r Employment by Loan Statue, echo=FALSE, message=FALSE, warning=FALSE}
