...
The majority of loans are for debt consolidation. Given such a high proportion of loans are for debt consolidation, not available or other, I won't be focussing on the variable to in this analysis.
...
According to the Prosper website the Prosper Rating assesses the estimated average annualized loss rate range with AA as the lowest risk and HR as the highest risk. 
...
```{r Prosper Rating and Score, echo=FALSE, message=FALSE, warning=FALSE}
...
The lender yield is equal to the interest rate on the loan less the servicing fee. As such, it is not surprising to discover a corresponding spike in the borrower rate just above the 30% rate. There may be a default borrower rate if certain criteria a met resulting in the spike.

```{r Lender Yield and Borrower Rate, echo=FALSE, message=FALSE, warning=FALSE}
...
There are a number of loan characteristics that are categorised as having a higher risk. The analysis so far has shown that loans with a higher risk rating have a higher proportion of loans with a status of charged off, past due and defaulted.

Employment status and debt to income ratio have an effect on both the rating and outcome of the loans.  Furthermore, the ratings seem to be accurately indicating the risk of loans with lower risk loans recording a higher proportion of completions.
...
```{r Income Range by Borrower and Lender Yield, echo=FALSE, message=FALSE, warning=FALSE}
...
by(loanData, loanData$ListingCategory, function(x) loanData$ListingCategory/sum(loanData$IncomeRange))
...
```{r Lender Yield by Loan Status, echo=FALSE, warning=FALSE}
...
```{r Effective Yield vs Lender Yield, echo=FALSE, message=FALSE, warning=FALSE}
