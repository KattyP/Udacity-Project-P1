...
```{r Employment Duration, echo=FALSE, message=FALSE, warning=FALSE}
...
IncomeTable <- as_data_frame(with(subset(loanData), table(ListingCategory, IncomeRange)))
...
IncomeTable2
#Sum to 1 test - without rounding adds to 100
#IncomeTable2 %>%
...
There is a significant increase in the number of charged off and defaulted loans as the lender yield increases
...
```{r Employment Duration by Loan Status, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(aes(x = EmploymentStatusDuration/12), data = subset(loanData, LoanCategory != "NA" & EmploymentCategory != "NA")) +
  ...
  xlab("Duration in Years") +
  facet_wrap(~EmploymentCategory, scales = "free")
